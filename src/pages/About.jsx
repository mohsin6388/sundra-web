...
export default function AboutSections({ lang = "en" }) {
  ...
  const PRODUCTS_HIGHLIGHT = [
    {
      name: t("about.p1.name"),
      badge: t("about.p1.badge"),
      desc: t("about.p1.desc"),
      Icon: Wheat,
    },
    {
      name: t("about.p2.name"),
      badge: t("about.p2.badge"),
      desc: t("about.p2.desc"),
      Icon: TrendingUp,
    },
    {
      name: t("about.p3.name"),
      badge: t("about.p3.badge"),
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .hero-banner {
          background: linear-gradient(135deg, #0d1f11 0%, #1a3d20 50%, #0d1f11 100%);
          padding: 100px 30px; text-align: center; position: relative; overflow: hidden;
        }
        .hero-banner::before {
        ...
        .hero-title {
          font-family: 'Playfair Display', serif;
          font-size: clamp(2.4rem, 6vw, 64px);
          color: #fff; line-height: 1.15; margin: 0 0 20px; font-weight: 900;
        }
        .hero-subtitle {
        ...
        .stats-bar { background: #c9871f; padding: 40px 30px; display: flex; align-items: center; justify-content: center; }
        .stats-inner {
          max-width: 1100px; margin: 0 auto;
          display: grid; grid-template-columns: repeat(3, 1fr); gap: 200px;
        }
        ...
        .stat-label { font-size: 12px; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; }
        .mission-section { background: #f5f0e8; padding: 90px 30px; position: relative; overflow: hidden; }
        .section-inner { max-width: 1200px; margin: 0 auto; }
        .mission-grid { display: grid; grid-template-columns: 1.15fr 1fr; gap: 70px; align-items: center; }
        .values-mini-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 36px; }
        .value-card {
          background: rgba(255,255,255,0.9); border-radius: 18px; padding: 22px;
          border-top: 3px solid #c9871f; transition: transform 0.2s ease, box-shadow 0.2s ease;
        }
        .value-card:hover { transform: translateY(-4px); box-shadow: 0 12px 40px rgba(0,0,0,0.1); }
        .mission-img-wrap { position: relative; max-width: 460px; margin: 0 auto; }
        .mission-img { width: 100%; height: 480px; object-fit: cover; border-radius: 28px; display: block; box-shadow: 0 24px 70px rgba(0,0,0,0.2); }
        .img-overlay {
          position: absolute; bottom: 0; left: 0; right: 0; border-radius: 0 0 28px 28px;
          background: linear-gradient(to top, rgba(26,46,30,0.93) 0%, transparent 100%);
          padding: 52px 28px 28px;
        }
        .img-badge {
          ...
          align-items: center; gap: 10px; box-shadow: 0 8px 28px rgba(0,0,0,0.15);
        }
        .journey-section { background: #fff; padding: 90px 30px; }
        .section-header { text-align: center; margin-bottom: 60px; }
        .section-tag { color: #c9871f; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; font-size: 12px; margin-bottom: 14px; }
        ...
        .timeline::before {
          ...
          width: 2px; background: linear-gradient(to bottom, #c9871f, #1f4d2c); transform: translateX(-50%);
        }
        .timeline-item { display: grid; grid-template-columns: 1fr 60px 1fr; gap: 0; margin-bottom: 48px; position: relative; }
        .timeline-item:last-child { margin-bottom: 0; }
        .timeline-left { text-align: right; padding-right: 40px; padding-top: 4px; }
        .timeline-right { padding-left: 40px; padding-top: 4px; }
        .timeline-center { display: flex; flex-direction: column; align-items: center; }
        .timeline-dot {
          width: 44px; height: 44px; border-radius: 50%; background: #c9871f;
        ...
        .products-section { background: #f5f0e8; padding: 90px 30px; }
        .products-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 28px; margin-top: 16px; }
        .product-card {
        ...
        .product-card::before {
          ...
          height: 4px; background: linear-gradient(90deg, #c9871f, #1f4d2c);
        }
        .product-card:hover { transform: translateY(-6px); box-shadow: 0 20px 60px rgba(0,0,0,0.1); }
        .product-badge {
          display: inline-block; background: rgba(201,135,31,0.12); color: #c9871f;
          font-size: 10px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase;
          padding: 4px 12px; border-radius: 50px; margin-bottom: 18px;
        }
        .product-icon-wrap {
          width: 52px; height: 52px; border-radius: 14px; background: rgba(31,77,44,0.08);
          display: flex; align-items: center; justify-content: center; margin-bottom: 18px;
        }
        .product-name { font-family: 'Playfair Display', serif; font-size: 20px; font-weight: 700; color: #1a2e1e; margin: 0 0 10px; }
        .product-desc { font-size: 14px; color: #666; line-height: 1.75; margin: 0 0 24px; }
        .facility-section { background: #fff; padding: 90px 30px; }
        .facility-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 70px; align-items: center; }
        .specs-row { display: flex; gap: 28px; flex-wrap: wrap; margin: 28px 0 36px; }
        .spec-item { border-left: 3px solid #c9871f; padding-left: 14px; }
        ...
        .cta-title { font-family: 'Playfair Display', serif; font-size: clamp(2rem, 4.5vw, 52px); color: #fff; font-weight: 900; line-height: 1.2; margin: 0 0 18px; }
        .cta-desc { color: rgba(255,255,255,0.8); font-size: 16px; line-height: 1.8; max-width: 500px; margin: 0 auto 40px; }
        .cta-contact-row { display: flex; justify-content: center; gap: 32px; margin-bottom: 36px; flex-wrap: wrap; }
        .contact-pill {
          display: inline-flex; align-items: center; gap: 10px;
          background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3);
          border-radius: 50px; padding: 10px 22px; color: #fff; font-size: 14px; font-weight: 600; text-decoration: none;
        }
        .btn-primary {
          display: inline-flex; align-items: center; gap: 10px; background: #1f4d2c;
        ...
        .btn-primary:hover { background: #163a21; transform: translateY(-2px); }
        .btn-gold {
          display: inline-flex; align-items: center; gap: 10px; background: #c9871f;
          color: #fff; text-decoration: none; padding: 14px 32px; border-radius: 50px;
          font-weight: 700; font-size: 14px; box-shadow: 0 8px 28px rgba(201,135,31,0.35);
          transition: opacity 0.2s ease, transform 0.2s ease;
        }
        .btn-gold:hover { opacity: 0.9; transform: translateY(-2px); }
        .btn-outline {
          display: inline-flex; align-items: center; gap: 10px; background: transparent;
          color: #fff; text-decoration: none; padding: 13px 30px; border-radius: 50px;
          font-weight: 600; font-size: 14px; border: 1.5px solid rgba(255,255,255,0.4);
          transition: background 0.2s ease;
        }
        .btn-outline:hover { background: rgba(255,255,255,0.08); }
        @media (max-width: 960px) {
          .mission-grid, .facility-grid { grid-template-columns: 1fr !important; gap: 40px !important; }
          .mission-grid > div:last-child { order: -1; }
          .timeline::before { left: 24px; }
          .timeline-item { grid-template-columns: 48px 1fr; }
          .timeline-left { display: none; }
      ...
      <div className="about-page">
        ...
        <section className="hero-banner">
          ...
          >
            ...
            <h1 className="hero-title">
              <Gold>{t("about.hero.title_1")}</Gold>
              <br />
              {t("about.hero.title_2")}
            </h1>
            <p className="hero-subtitle">{t("about.hero.desc")}</p>
            <div className="hero-links">
              <Link to="/products" className="btn-gold">
                {t("about.hero.cta1")} <ArrowRight size={16} />
              </Link>
              <Link to="/contact" className="btn-outline">
                {t("about.hero.cta2")} <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="products-section">
          <div className="section-inner">
            <div className="section-header">
              ...
              <h2 className="section-title">{t("about.products.title")}</h2>
              <p className="section-desc">{t("about.products.desc")}</p>
            </div>
            <div className="products-grid">
              {PRODUCTS_HIGHLIGHT.map(({ name, badge, desc, Icon }) => (
                <div key={name} className="product-card">
            ...
            <div style={{ textAlign: "center", marginTop: "44px" }}>
              <Link to="/products" className="btn-primary">
                {t("about.products.cta")} <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
          ...
          <div className="section-inner">
            <div className="facility-grid">
              ...
              <div>
                ...
                <h2 className="section-title">{t("about.facility.title")}</h2>
                <p
                  style={{
                    fontSize: "15px",
                    lineHeight: "1.85",
